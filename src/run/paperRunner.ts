// Paper runner (Phase 4) — selection engine + strategy registry + control panel.
//
// Flow: discover reward markets -> score & select top N (>= act score) -> open
// parallel WS over chosen YES tokens -> each cycle pick the best enabled strategy
// per market, gate risk, reconcile quotes, manage inventory, accrue calibrated
// rewards, and fire per-strategy Telegram alerts. Settings are read live so the
// Telegram panel changes behaviour without a restart.
...
import { reconcileQuotes, buildInventoryExit, netExposureUsd, canQuote, DriftDetector, AdverseSelectionMonitor } from "../strategy/index.js";
...
async function requoteCycle(
	...
	for (const ctx of ctxs) {
		...
		// Cancel quotes from a different (switched) strategy too.
		for (const o of paper.getOpenOrders(token)) if (o.strategy !== out.strategy && o.strategy !== "inventory-exit") paper.cancelOrder(o.id);
